...
export default function AgendaPage() {
  ...
  const [agSelecionado, setAgSelecionado] = useState<AgendamentoView | null>(null)
  ...
  const carregar = useCallback(async () => {
    ...
    setAgendamentos((data as AgendamentoView[]) || [])
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        const lista = (data || []) as Dentista[]
        setDentistas(lista)
        setDentistasFiltro(new Set(lista.map((d: Dentista) => d.id)))
      })
  }, [clinica])
  ...
  function clicarSlot(dia: Date, hora: number) {
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <div className="h-11 flex items-center px-4 gap-3 border-b border-black/[0.06] bg-white flex-shrink-0">
        ...
        <div className="flex items-center gap-1">
          <button onClick={() => setSemanaBase(subWeeks(semanaBase, 1))}
          ...
          <button onClick={() => setSemanaBase(addWeeks(semanaBase, 1))}
            className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-gray-100 transition-colors text-gray-500">
          ...
          <div className="mt-4">
            ...
            <div className="space-y-1.5">
              {dentistas.map((d, i) => {
                ...
                return (
                  <button key={d.id} onClick={() => toggleDentista(d.id)}
              ...
              return (
                <div key={dia.toISOString()} className="flex-1 border-l border-black/[0.04] min-w-0">
                  ...
                  <div className="relative" style={{ height: HORAS.length * SLOT_HEIGHT }}>
                    {HORAS.map(h => (
                      <div key={h} onClick={() => clicarSlot(dia, h)}
      ...
      {modalAberto && (
        <ModalAgendamento
          ...
          agendamento={agSelecionado as any}
          dentistas={dentistas as any}
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        ...
        <span className="text-[11px] font-medium">{format(mes, 'MMM yyyy', { locale: ptBR })}</span>
        <button onClick={() => setMes(d => new Date(d.getFullYear(), d.getMonth() + 1, 1))}
          className="w-5 h-5 flex items-center justify-center rounded hover:bg-gray-100 text-gray-500">
