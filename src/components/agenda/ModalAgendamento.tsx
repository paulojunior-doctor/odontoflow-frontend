...
import type { Agendamento, Usuario, Contato } from '@/types'
            ...
            {contatoSelecionado ? (
              <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[#E1F5EE] border border-[#1D9E75]/20">
                ...
                <button onClick={() => { setContatoSelecionado(null); setBuscaContato('') }}
                  className="text-[#1D9E75] hover:text-[#085041]">
                  <X size={14} />
                </button>
              </div>
            ) : (
                ...
                {resultados.length > 0 && (
                  <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-black/[0.08] rounded-xl shadow-lg z-10 overflow-hidden">
                    {resultados.map(c => (
                      <button
                        key={c.id}
                        onClick={() => { setContatoSelecionado(c); setResultados([]) }}
                      ...
                      >
                        ...
                        {c.interesse && (
                          <span className="text-[10px] text-[#1D9E75]">{c.interesse}</span>
                        )}
                      </button>
                    ))}
                  </div>
